    ...
    for chaocan in range(1):
        ...
        for i in range(1):
            ...
            for file in files:
                ...
                if daimafangshi == 1:
                    print(f"这里为固定参数110")
                    epoch, all_reduce_molecule_numbers, listmo, test_accuracy = TRAIN(file, n, similer_nuber, bili,sim_baifenbi=0, randon_baifenbi=0,savepth=file.replace('train', 'pth') +'randon_number-'+str(n)+'similer_nuber-'+str(similer_nuber)+ '_xr_noECFP.pth')
                elif daimafangshi == 0:
                    print(f"这里为固定比例")
                    epoch, all_reduce_molecule_numbers, listmo, test_accuracy = TRAIN(file, n, similer_nuber, bili,sim_baifenbi=0, randon_baifenbi=0,savepth=file.replace('train', 'pth') +'randon_number-'+str(n)+'similer_nuber-'+str(similer_nuber)+ '_xr_noECFP.pth')
